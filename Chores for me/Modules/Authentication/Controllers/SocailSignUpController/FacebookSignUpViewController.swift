...
//  Chores for me
//
//  Created by Ios Mac on 28/02/22.
//
...
class FacebookSignUpViewController: BaseViewController {
    ...
    private func callingSignUpAPI() {
        ...
        let parameterDictionary = ["name": RegisterViewController.userName ?? "","email": RegisterViewController.gmailName ?? "","image": UserStoreSingleton.shared.socailProfileImage ?? "","phone": phoneNumber,"socialKey": RegisterViewController.socailKey ?? "","signupType":"0"] as [String: Any]
...
//                            verificationTextField.text = getOtp
    ...
    @IBAction func didTappedSignUpButton(_ sender: UIButton) {
        if let mobile = mobileNumberTextFeild.text, let image =  UIImage(named: "Unchecked-Checkbox-256") {
          if mobile == "" {
...
extension FacebookSignUpViewController: ADCountryPickerDelegate{
